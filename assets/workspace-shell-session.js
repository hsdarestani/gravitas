...
var theme=document.getElementById('ws-theme');if(theme)theme.addEventListener('click',function(){var next=document.documentElement.dataset.theme==='dark'?'light':'dark';this.setAttribute('aria-pressed',String(next==='light'));document.documentElement.dataset.theme=next;localStorage.setItem('gravitas-theme',next)});
csrf().then(me).then(function(d){if(!d.authenticated){location.href='/login';return}var user=document.getElementById('ws-user'),avatar=document.getElementById('ws-avatar');if(user){var nm=(d.user.name||'').trim();user.textContent=nm||(d.user.email||'').split('@')[0]||'Account';var sm=user.closest&&user.closest('summary');if(sm&&d.user.email)sm.setAttribute('title',d.user.email);}if(avatar)avatar.textContent=(d.user.name||d.user.email||'G').charAt(0).toUpperCase()}).catch(function(){location.href='/login'});
