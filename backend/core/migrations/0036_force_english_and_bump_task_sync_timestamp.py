...
def force_english_and_bump_updated_at(apps, schema_editor):
    OperatingTask = apps.get_model('core', 'OperatingTask')
    db_alias = schema_editor.connection.alias

    tasks = list(
        OperatingTask.objects.using(db_alias)
        .filter(description__startswith=f'[{SPRINT_MARKER}]', status='active')
        .order_by('board_order', 'id')
    )

    by_order = {task.board_order: task for task in tasks}

    for board_order, (title, definition_of_done) in ENGLISH_COPY.items():
        task = by_order.get(board_order)
        if task is None:
            continue
        task.title = title
        task.definition_of_done = definition_of_done
        # Use model save rather than QuerySet.update so auto_now updates the
        # sync timestamp. That prevents an older Persian Nextcloud Deck card
        # from being pulled back into Gravitas on the next automatic sync.
        task.save(
            using=db_alias,
            update_fields=['title', 'definition_of_done', 'updated_at'],
        )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('core', '0035_force_sprint_one_english_copy'),
    ]

    operations = [
        migrations.RunPython(
            force_english_and_bump_updated_at,
