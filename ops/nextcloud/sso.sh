...
systemctl restart gravitas-backend.service
for _ in $(seq 1 60); do
  if curl -fsS "$DISCOVERY" >/tmp/gravitas-oidc-discovery.json 2>/dev/null \
     && curl -fsS "${ISSUER}/jwks/" >/tmp/gravitas-oidc-jwks.json 2>/dev/null; then
    break
  fi
  sleep 2
done
grep -q '"authorization_endpoint"' /tmp/gravitas-oidc-discovery.json
grep -q '"RS256"' /tmp/gravitas-oidc-jwks.json
...
curl -fsS "$DISCOVERY" | grep -q '"gravitas-nextcloud"' || true
